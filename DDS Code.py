...
# The connections to the AD9837 eval board are:
# FSYNC -> 2
# SCLK -> 13 (SCK)
# SDATA -> 11 (MOSI)
# +Vin = VCC
...
def setup():
    spi = SPI(0, baudrate = 9600)
    cs = Pin(1, mode = Pin.OUT, value = 1)
    utime.sleep.ms(100)
    freq = 63000
    WriteFrequencyAD9837(freq)
    cs(0)
    spi.write("Frequency is ")
    spi.write(freq)
    spi.write("")
...
def WriteFrequencyAD9837(frequency):
    int MSB
    int LSB
    int phase
    float AD9837Val
    AD9837Val = float(frequency/16000000)
    calculated_freq_word = AD9837Val*0x1000000
    MSB = int(calculated_freq_word
